...
class MyArtificialNeuralNetwork(object):
    ...
    def __init__(self, monthToRead, data, pfad=None, tensorboardlogging=False):
        ...
        with tf.name_scope('Model'):
            ...
            #self.model, self.w1, self.b1, self.maskedw1 = mdl.add_layer(self.x, self.input.train.x[1].size, 4, tf.tanh, "Layer1", self.input.maske)#mit Maske
            #keep_prob = tf.placeholder("float")
            #model = tf.nn.dropout(model, keep_prob)
            self.model, self.w2, self.b2, self.maskedw2 = mdl.add_layer(self.model, 4, 1, layer_name="Layer2")
...
=================================================================================================================
    ...
    def startTraining(self):
        ...
        while True:
            self.i += 1
            self.summary, curr_trainstep, self.loss_train, curr_w1, curr_w2, curr_b1, curr_b2 = self.sess.run([self.merged, self.train_step, self.loss, self.maskedw1, self.w2, self.b1, self.b2], feed_dict={self.x: self.input.train.x, self.y: self.input.train.y, self.a: self.input.train.a, self.b: self.input.train.b})
            ...
            elif (self.i > 5000 and (self.loss_valid) > self.absoluteMinimum*1.30) and self.loss_train < self.loss_valid  or self.i >= 250000:# or loss_delta_counter == 0:
            #if i > 5000:
                pass
                self.patience_cnt += 1
