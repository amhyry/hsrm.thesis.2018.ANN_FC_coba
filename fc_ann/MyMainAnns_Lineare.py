...
import version14.Modeller as mdl
import version14.Library as mylib
#from version13.Dialogbox import Processing
...
class MyArtificialNeuralNetwork(object):
    ...
    def __init__(self, monthToRead, data, pfad=None, tensorboardlogging=False):
        ...
        #self.input.normalize_all(mylib.NormFunction)
        ...
        with tf.name_scope('Model'):
            ...
            #self.model, self.w1, self.b1, self.maskedw1 = mdl.add_layer(self.x, self.input.train.x[1].size, 4, tf.tanh, layer_name="Layer1")
            self.model, self.w1, self.b1, self.maskedw1 = mdl.add_layer(self.x, self.input.train.x[1].size, 1, layer_name="Layer1")#mit Maske
            #keep_prob = tf.placeholder("float")
            #model = tf.nn.dropout(model, keep_prob)
            #self.model, self.w2, self.b2, self.maskedw2 = mdl.add_layer(self.model, 4, 1,tf.tanh, layer_name="Layer2")
        ...
        with tf.name_scope('loss'):
            ...
            self.loss = tf.reduce_sum(tf.square(self.model - self.y))
        ...
        with tf.name_scope('Measures'):
            ...
            self.mse_loss = tf.losses.mean_squared_error(labels=self.y, predictions=self.y_)
            self.sse_loss = tf.reduce_sum(tf.square(self.y_ - self.y))
            
            self.mse = tf.metrics.mean_squared_error(labels=self.y_, predictions=self.model, name="MSE")
            self.mae = tf.metrics.mean_absolute_error(labels=self.y_, predictions=self.model, name="MAE") 
...
#==============================================================================
        ...
        self.absoluteMinimum = self.absoluteMinimum + self.loss.eval(session=self.sess, feed_dict={self.x: self.input.train.x, self.y: self.input.train.y, self.a: self.input.train.a, self.b: self.input.train.b})
...
=================================================================================================================
    ...
    def startTraining(self):
        ...
        while True:
            self.i += 1
            self.summary, curr_trainstep, self.loss_train, curr_w1, curr_b1 = self.sess.run([self.merged, self.train_step, self.loss, self.maskedw1, self.b1], feed_dict={self.x: self.input.train.x, self.y: self.input.train.y, self.a: self.input.train.a, self.b: self.input.train.b})
            #summary, curr_trainstep, self.loss_train, curr_w1, curr_w2, curr_b1, curr_b2 = self.sess.run([self.merged, self.train_step, self.loss, self.maskedw1, self.w2, self.b1, self.b2], feed_dict={self.x: self.input.train.x, self.y: self.input.train.y, self.a: self.input.train.a, self.b: self.input.train.b})
            ...
            if self.loss_valid_absoluteMinimum > self.loss_valid:
                self.loss_valid_absoluteMinimum = self.loss_valid
            
            #finde absolutes Minimum, f�hre saverfunktion durch.
            if self.absoluteMinimum > self.loss_valid + self.loss_train:# and loss_delta_minimum > (loss_valid - loss_train):
            #if absoluteMinimum > loss_valid and loss_delta_minimum > (loss_valid - loss_train):
                self.absoluteMinimum = self.loss_valid + self.loss_train
                
                storedw1 = curr_w1
                #storedw2 = curr_w2
                storedb1 = curr_b1
                #storedb2 = curr_b2
                self.storedAtStep = self.i
            ...
            elif (self.i > 5000 and (self.loss_valid) > self.absoluteMinimum*1.30) and self.loss_train < self.loss_valid  or self.i >= 10000:# or loss_delta_counter == 0:
            #if i > 5000:
                pass
                self.patience_cnt += 1
            ...
            if self.patience_cnt > self.patience or self.forced_button == True:
                ...
                #self.maskedw1 =  tf.multiply(storedw1 , self.input.maske) 
                #updatew1 = tf.assign(self.w1, tf.multiply(storedw1 , self.input.maske))
                
                #self.maskedw1 = tf.Variable(tf.zeros(self.w1.shape))
                
                #init_new_vars_op = tf.variables_initializer([self.maskedw1])
                #self.sess.run(init_new_vars_op)
                #storedw1 = tf.zeros(storedw1.shape, dtype=tf.float64)
            ...
            if self.i % 1000 == 0:
                ...
                self.logger_array_training.append([self.i, self.loss_train, curr_w1])
...
=================================================================================================================
        ...
        self.output.train.y_ = self.sess.run(self.model, feed_dict={self.x:  self.output.train.x, self.a: self.input.train.a, self.b: self.input.train.b})
        self.output.validation.y_ = self.sess.run(self.model, feed_dict={self.x:  self.output.validation.x, self.a: self.input.validation.a, self.b: self.input.validation.b})
        self.output.test.y_ = self.sess.run(self.model, feed_dict={self.x:  self.output.test.x, self.a: self.input.test.a, self.b: self.input.test.b})

        #self.output.denormalize_all(mylib.DeNormFunction)
        #self.output.denormalize_all_out(mylib.DeNormFunction)
...
=================================================================================================================        
        ...
        np.savetxt(self.pfad + "/errors_loss.csv", mylib.convertForCSV(np.asarray(self.errors_trainingdata)), delimiter=";",fmt='%s')
        np.savetxt(self.pfad + "/errors_valid.csv", mylib.convertForCSV(np.asarray(self.errors_validationdata)), delimiter=";",fmt='%s')
        
        np.savetxt(self.pfad + "/W1.csv", mylib.convertForCSV(np.asarray(self.sess.run(self.w1))), delimiter=";",fmt='%s')
        #np.savetxt(self.pfad + "/W2.csv", mylib.convertForCSV(np.asarray(self.sess.run(self.w2))), delimiter=";",fmt='%s')
        #np.savetxt(self.pfad + "/B1.csv", np.asarray(self.sess.run(self.b1)), delimiter=";",fmt='%10.5f')
        #np.savetxt(self.pfad + "/B1.csv", np.asarray(self.sess.run(self.b2)), delimiter=";",fmt='%10.5f')        
        ...
        logging.info('Gespeichert at: %g ' % (self.storedAtStep))
        logging.info(self.mse_loss.eval(session=self.sess, feed_dict={self.y: self.output.train.y, self.y_: self.output.train.y_}))
        logging.info(self.mse_loss.eval(session=self.sess, feed_dict={self.y: self.output.validation.y, self.y_: self.output.validation.y_}))
        logging.info(self.mse_loss.eval(session=self.sess, feed_dict={self.y: self.output.test.y, self.y_: self.output.test.y_}))
        
                
        logging.info("Die Losswerte der Optimalen Funktion:")
        self.train_loss = self.mse_loss.eval(session=self.sess, feed_dict={self.y: self.output.train.y, self.y_: self.output.train.y_})
        self.valid_loss = self.mse_loss.eval(session=self.sess, feed_dict={self.y: self.output.validation.y, self.y_: self.output.validation.y_})
        self.test_loss = self.mse_loss.eval(session=self.sess, feed_dict={self.y: self.output.test.y, self.y_: self.output.test.y_})
        
        self.train_loss_sse = self.sse_loss.eval(session=self.sess, feed_dict={self.y: self.output.train.y, self.y_: self.output.train.y_})
        self.valid_loss_sse = self.sse_loss.eval(session=self.sess, feed_dict={self.y: self.output.validation.y, self.y_: self.output.validation.y_})
        self.test_loss_sse = self.sse_loss.eval(session=self.sess, feed_dict={self.y: self.output.test.y, self.y_: self.output.test.y_})
        
        #mse_t = self.sess.run(self.mse_loss, feed_dict={self.x: self.output.train.x, self.y_: self.input.train.y, self.a: self.input.train.a, self.b: self.input.train.b})
        #mse_v = self.sess.run(self.mse_loss, feed_dict={self.x: self.output.train.x, self.y_: self.input.validation.y, self.a: self.input.validation.a, self.b: self.input.validation.b})
        #mse_tes = self.sess.run(self.mse_loss, feed_dict={self.x: self.output.train.x, self.y_: self.input.test.y, self.a: self.input.test.a, self.b: self.input.test.b})
